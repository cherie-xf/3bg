	...
	<div class="modal fade" id="confirm-message" tabindex="-1"
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<a href="suppliermanage.html" class="btn btn-default" >确定</a>					
				</div>
			</div>
		</div>
	</div>	
								      ...
								      <tr>  
									       ...
									       <td>
												<c:if test="${var.status_id=='10' || var.status_id=='20' || var.status_id=='30'}"> 
													<a data-href="suppliercancel.html?cancelOrderId=${var.order_id}" data-toggle="modal" data-target="#confirm-cancel" href="#" id="cancel-operate" title="${var.order_id}">取消</a>
												</c:if>
												<c:if test="${var.status_id=='10' || var.status_id=='30'}">
													<a class="openEditPage" href="bid.html?order_id=${var.order_id}">接单</a>
												</c:if> 
											</td>			       
								      </tr>
								    </c:forEach>
	...
	<script type="text/javascript" src="/js/tinymce/tinymceinit.js"></script>
     ...
     $(document).ready(function(){
	    ...
	    showMessage();
	 	...
	 	$('#backendMessage').change(function(){
			var id = $(this).attr('title');			
			$('#confirm-message').on('show.bs.modal', function(e) {
			    $(this).find('.danger').attr('href', $(e.relatedTarget).data('href'));
			    $('.info').html($('#backendMessage').val());
			});
