			...
			<div class="row">
	           <div class="col-sm-12">
		           <div class="form-group">	             
		              <h2 style="text-align:left; padding:10px">订单详情</h2>	             
		           </div>
	           </div>
          	 </div>          	 
			 <div class="row myrow">						
				 ...
				 <table class="table">
				      <tr valign="middle">  
				       ...
				       <td>
				       		<div class="row myrow">
				       			<p id="p_unit_price">
				       			单价： $${serviceOrder.unitPrice} / ${serviceOrder.unit_name} * ${serviceOrder.unit_quantity}
				       			</p>
				       		</div>				         	 
				       </td>
				       <td class="text-right">
							...
							<fmt:formatDate value="${var.servicetime}" var="timePartial" pattern="h:mm a"/>
	               			<div class="form-inline">			
	                			<form:label path="selectedServiceTimeIndex" class="radio-inline">
										<form:radiobutton path="selectedServiceTimeIndex" name="servicetimeradios" value="${rowCounter.index}" disabled="true"/>
								</form:label>
								<form:input path="serviceScheduleList[${rowCounter.index}].datePartial" value = "${datePartial}" cssClass="form-control_inline input-sm" id="datepicker${rowCounter.index}" placeholder="日期" 
	               					data-validation-optional="true" data-validation="date" data-validation-format="MM/dd/YYYY" disabled="true"/>
							    <form:input path="serviceScheduleList[${rowCounter.index}].timePartial" value = "${timePartial}" size="5" cssClass="input-sm form-control_inline" id="timepicker${rowCounter.index}" placeholder="时间" 
							    	data-validation-optional="true" data-validation="time" disabled="true"/>
							</div>									
						</div>
					</div>							
				</c:forEach>						
